...
class TestPrecommitArtifactE2E:
    """Schema v2 (#123 D4): .pre-commit-config.yaml is a pure artifact.

    Any hand edit is overwritten on the next install. Projects wanting
    persistent external repos must declare them in guard.yaml under
    ``code.<stage>.hooks`` — they're then re-rendered deterministically.
    """

    def test_update_overwrites_user_edits(self, tmp_path: Path) -> None:
        config = _write_config(tmp_path)
        (tmp_path / ".git").mkdir()
        runner.invoke(app, ["install", "-a", "claude-code", "-c", str(config)])
        ...
        assert "# Generated by ac-guard install" in original

        # Hand-edit: add a repo and inject a comment. Both must disappear
        # on the next install since the file is now a pure artifact.
        ...
        assert "hand edit survives?" not in updated
        assert "PyCQA/bandit" not in updated
        assert updated.startswith("# Generated by ac-guard install")
