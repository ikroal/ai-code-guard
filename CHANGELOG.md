...
- **Generated hooks now bake absolute interpreter paths.** Previously
  the 5 git hook templates (pre-commit / pre-push / commit-msg /
  pre-merge-commit / pre-rebase) called `ac-guard run` and the cursor /
  opencode hook templates called `python3 -m ac_guard.action_guard` —
  both relying on the project venv being on `$PATH`. Callers that don't
  activate the venv (Claude Code's `PreToolUse` hook, generic CI
  runners, IDEs that don't auto-source venv) hit `command not found`
  and silently skipped the gate.

  `ac-guard install` and `ac-guard update` now resolve the absolute
  path of `ac-guard` (next to `sys.executable`, with `shutil.which`
  fall-back) and `python3` (`sys.executable` directly), and bake those
  into the generated hook scripts. Hooks then `exec` the absolute
  paths with no PATH dependency. Both commands print the baked path
  (`Hooks linked to: /abs/path/ac-guard`) so users can tell which
  install the hooks point at.

  No user action required if hooks are regenerated by the next
  `ac-guard install`/`update`.
